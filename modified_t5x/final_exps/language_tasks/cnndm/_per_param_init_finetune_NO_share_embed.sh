...
# dont run this. always should share embed. better performance.
exit 1
...
if [ "$IDK" = "lm" ]
then
python extract_results/avg_metric.py $exp_logdir/inference_eval/$MTN-metrics.jsonl 1,0 rouge1
exit 1
fi


python ./t5x/train.py \
--gin_file=t5x/configs/final_exps/t5/finetune/small_no_share_embed.gin \
