...
for task in taskss:
  # concat
  concat_path = f'cfg_tasks/generated_cfg_task_data/mar20/{task}/all_processes_1100000_data_concat_{task}.txt'
  assert osp.isdir(osp.dirname(concat_path))
  command = f'python cfg_tasks/generate_data_scripts/concat_data.py \
  cfg_tasks/generated_cfg_task_data/mar20/{task}/\*process\*generated\*.txt {concat_path}'
  print(command)
  bash(command)
